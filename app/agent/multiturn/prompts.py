...
MULTI_TURN_PROMPT = (
    ...
    "- On the second turn of the conversation, ask the shop-focused question 'Please let "
    "me know what kind of shop you have in mind, warranty and score all help narrow it "
    "down.' and record it as 'shop_expectations' in asked_fields or excluded_fields.\n"
    "- Do not present numbered candidates or return a member key until both "
    "product_overview and shop_expectations have been recorded.\n"
    "- When the user selects one of the last_options by its number, return the "
    "matching member_random_key and set done to true.\n"
    "- Always call the search_members tool to gather results, but only after you have "
    "asked the mandatory clarification questions for the current conversation. Invoke "
    "it at most once per turn; only perform a second call if the first returns count = "
    "0 and you need to run one relaxation step. Apply relaxation in this exact order "
    "and advance the stage by one: 1) trim low-value query_tokens, 2) widen the price "
    "range, 3) drop brand_id, 4) drop city_id, 5) ignore has_warranty. Do not apply "
    "more than one relaxation step per turn.\n"
